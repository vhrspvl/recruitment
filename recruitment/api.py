...
@frappe.whitelist()
def confirm_register(candidate_name,testid):
 candidate = frappe.db.get_value("Test Registration", candidate_name, "name")
 get_candidate = frappe.get_doc("Test Registration",candidate)
 get_token = frappe.db.get_value("Token Summary", testid, "name")
 token = frappe.get_doc("Token Summary",get_token)
 if get_candidate.test_id == '123123':
  frappe.db.set_value("Test Registration", candidate, "test_id", testid)
  frappe.db.set_value("Test Registration", candidate, "status", "Registered")
  frappe.db.set_value("Token Summary",token,"validity","Invalid")
  return testid
 else:
...
def fetch_candidate(project,payment_type):
    if payment_type == 'client':
        candidate = frappe.get_list("Closure", fields=["name1","contact_no"] , filters={"docstatus":1, "project":project,"client_payment_applicable":1}, order_by= "idx")
    else:
        candidate = frappe.get_list("Closure", fields=["name1","contact_no"] , filters={"docstatus":1, "project":project,"candidate_payment_applicable":1}, order_by= "idx")
    return candidate
	...
	if work_anniversary:
		...
		for e in work_anniversary:
			frappe.sendmail(recipients=filter(lambda u: u not in (e.company_email, e.personal_email, e.user_id), users),
				subject=_("Work Anniversary Reminder for {0}").format(e.employee_name),
				message=_("""Today is {0}'s Work Anniversary!""").format(e.employee_name),
...
def get_bdm_users(doctype, txt, searchfield, start, page_len, filters):
	user_list = frappe.db.sql("""select user.name,user.full_name,userrole.parent from tabUserRole userrole,tabUser user where userrole.role = 'Sales User' and userrole.role != 'Administrator' and user.enabled and user.name=userrole.parent""")
	return user_list
...
def get_dle(doctype, txt, searchfield, start, page_len, filters):
	...
	dle = 	frappe.db.sql("""select candidate.executive from tabCandidate candidate where candidate.name = %s""", (filters.get("candidate")))
